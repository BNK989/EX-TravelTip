...
function addUpdateLocation(ev) {
  ev.preventDefault()

  const dialog = document.querySelector('dialog')
  const locName = dialog.querySelector('#name').value
  const locRating = dialog.querySelector('#rating').value
...
function onSetSortBy() {
  ...
  const sortBy = {}
  sortBy[onSetFilterBy] = isDesc ? -1 : 1
